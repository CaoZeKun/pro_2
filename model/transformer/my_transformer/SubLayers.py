...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        sz_b, len_q, _ = q.size()  # dimension should be understand !!!???
        sz_b, len_k, _ = k.size()
        sz_b, len_v, _ = v.size()
        ...
        # q = q.permute(2, 0, 1, 3).reshape(-1, len_q, d_k)
        # permute (change sub seqence)
        k = k.permute(2, 0, 1, 3).contiguous().view(-1, len_k, d_k)  # (n*b) x lk x dk
        v = v.permute(2, 0, 1, 3).contiguous().view(-1, len_v, d_v)  # (n*b) x lv x dv

        mask = mask.repeat(n_head,1, 1)  # (n*b) x .. x ..  dimension???
        output, attn = self.attention(q, k, v, mask=mask)

        output = output.view(n_head, sz_b, len_q, d_v)
        output = output.permute(1, 2, 0, 3).contigious().view(sz_b, len_q, -1)  # b x lq x (n*dv)
