...
def data_process_predict(data_x, window_size=2):
    """
    目的：处理数据，使用连续的window_size个样本作为最后一个样本得特征，用于测试。
    :param data_x: 所有样本的特征
    :param window_size: 窗口大小
    :return: 样本特征 dataX
    """
    ...
    return np.array(dataX).astype(float)


def data_feature_predict(data, isBatchTes=False, window_size=1, BATCH_SIZE_TES=1, SHUFFLE_BOOL_TES=False,
                       NUM_WORKERS_TES = 0, DROP_LAST_TES = False):
    """

    :param data: 测试数据
    :param isBatchTes: 是否使用Batch
    :param window_size: 窗口大小
    :param BATCH_SIZE_TES: 测试集批处理量，默认：1
    :param SHUFFLE_BOOL_TES: 测试集是否打乱，默认：False （不打乱）
    :param NUM_WORKERS_TES: 测试集中用于数据加载的子进程数，默认：0
    ...
    """
    data = np.array(data)
    x_test = data_process_predict(data,window_size)
    # print(np.shape(x_test))
    x_test = torch.FloatTensor(x_test)
    if isBatchTes:
        test_data = Data.TensorDataset(x_test)
        test_loader = torch.utils.data.DataLoader(dataset=test_data,
                                             batch_size=BATCH_SIZE_TES,
                                             shuffle=SHUFFLE_BOOL_TES,
                                             num_workers=NUM_WORKERS_TES,
...
def load_model_test_data(PATH,data,isClassfier=True,isBatchTes=False,Seq=1,K_fea=1,CUDA_ID="0",BATCH_SIZE_TES=1,USE_CUDA=False):
    """
    目的：加载训练好的模型，进行测试/预测.（默认数据无标签列，且数据集中特征列与训练集一致）
    :param PATH:保存的模型路径
    :param data:测试数据
    :param isClassfier:是否为分类，默认True
    :param isBatchTes: 测试集是否使用Batch， 默认: False
    :param Seq: 时间序列数，默认：1  (预测时，前后数据不按时间序列排则 Seq应为1，否则可设window_size)
    :param K_fea: 特征的列数，默认：1
    :param CUDA_ID: GPU ID号，默认：0
    :param BATCH_SIZE_TES: 测试集批处理量，默认：1
    :param USE_CUDA:是否使用cuda，默认False
    ...
    model = torch.load(PATH)
    model.eval()
    data = data_feature_predict(data, isBatchTes, window_size=Seq, BATCH_SIZE_TES=BATCH_SIZE_TES)
    if isBatchTes:
        pred_y = torch.Tensor([])
        # if isClassfier:
        #     data_y = torch.LongTensor([])
...
if __name__ =='__main__':
    ...
    """         *** Test  Class***         """
    """ case1 """
    dataFram = pd.read_csv('iris1.data',header=None,)
    key_index = {dataFram.iat[0, i]: i for i in range(dataFram.shape[1])}
    isColumnName = True
    """case1 test 1"""
    data, k_fea = data_processing(dataFram,isColumnName, key_index = key_index)
